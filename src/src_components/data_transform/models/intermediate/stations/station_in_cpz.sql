...
select 
    ...
    case
        ...
        when abs(subtract(inside, outside)) >= 100 and inside > outside then True
        when abs(subtract(inside, outside)) >= 100 and inside < outside then False
        else null
     end as in_cpz
from pivoted